...
export default function PagesManager() {
  ...
  useEffect(() => {
    // FAQs
    const storedFaqs = localStorage.getItem('nadif_cms_faqs');
    if (storedFaqs) {
      setFaqs(JSON.parse(storedFaqs));
    } else {
      localStorage.setItem('nadif_cms_faqs', JSON.stringify(DEFAULT_FAQS));
      setFaqs(DEFAULT_FAQS);
    }

    // Privacy Policy
    const storedPrivacy = localStorage.getItem('nadif_cms_privacy');
    if (storedPrivacy) {
      setPrivacyPolicy(storedPrivacy);
    } else {
      localStorage.setItem('nadif_cms_privacy', DEFAULT_PRIVACY);
      setPrivacyPolicy(DEFAULT_PRIVACY);
    }

    // About Us
    const storedAbout = localStorage.getItem('nadif_cms_about');
    if (storedAbout) {
      setAboutData(JSON.parse(storedAbout));
    } else {
      localStorage.setItem('nadif_cms_about', JSON.stringify(DEFAULT_ABOUT));
  ...
  const saveFaqsToDb = (updated: FAQItem[]) => {
    setFaqs(updated);
    localStorage.setItem('nadif_cms_faqs', JSON.stringify(updated));
  };

  const savePrivacyToDb = (text: string) => {
    setPrivacyPolicy(text);
    localStorage.setItem('nadif_cms_privacy', text);
  };

  const saveAboutToDb = (updated: AboutUsData) => {
    setAboutData(updated);
    localStorage.setItem('nadif_cms_about', JSON.stringify(updated));
  };
