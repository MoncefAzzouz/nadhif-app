...
// Cleaner Interface (needed to compute assignments count)
...
export default function SkillsManager() {
  ...
  useEffect(() => {
    ...
    // 2. Load Cleaners list (to track dynamic assignments count)
    const storedCleaners = localStorage.getItem('nadif_cleaners_list');
    if (storedCleaners) {
      setCleaners(JSON.parse(storedCleaners));
    }
  ...
  const handleUpdateSkill = (e: React.FormEvent) => {
    ...
    if (oldSkill && oldSkill.name !== editingSkill.name) {
      const storedCleaners = localStorage.getItem('nadif_cleaners_list');
      if (storedCleaners) {
        const cleanersList: CleanerRecord[] = JSON.parse(storedCleaners);
        const updatedCleaners = cleanersList.map(c => {
          const updatedSkillsList = c.skills.map(s => s === oldSkill.name ? editingSkill.name : s);
          return { ...c, skills: updatedSkillsList };
        });
        localStorage.setItem('nadif_cleaners_list', JSON.stringify(updatedCleaners));
          ...
          return (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                className="bg-white border border-slate-100 rounded-[2.5rem] w-full max-w-lg p-8 shadow-2xl space-y-6 relative overflow-hidden max-h-[85vh] flex flex-col"
              >
                <button
                  onClick={() => setViewingWorkersSkill(null)}
                ...
                <div className="space-y-1.5 pr-8 shrink-0">
                  ...
                  <p className="text-xs text-slate-400 font-semibold font-inter leading-relaxed">
                    Below are the cleaning agents certified to perform **{viewingWorkersSkill.name}** operations in Sétif.
                  </p>
                </div>
                        ...
                        <div key={cleaner.id} className="p-4 bg-slate-50/50 border border-slate-100 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
                          ...
                          <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
                            ...
                            <span className={`px-2.5 py-1 rounded-lg text-[8.5px] font-black uppercase tracking-wider ${
                              ...
                              {cleaner.status === 'available' ? 'Disponible' : cleaner.status === 'on_duty' ? 'Intervention' : 'Inactif'}
                            </span>
                          </div>
                        </div>
                      );
