...
export default function CommandsPage() {
  ...
  const fetchCommands = async () => {
    try {
      const token = localStorage.getItem('nadif_admin_token');
      ...
      const mapped: Command[] = data.map((order: any) => {
        const names = order.user?.fullName?.split(' ') || ['Unknown'];
        const firstName = names[0];
        const lastName = names.slice(1).join(' ') || 'User';
        
        let statusMapped: 'pending' | 'approved' | 'completed' | 'cancelled' = 'pending';
        if (order.status === 'PENDING') statusMapped = 'pending';
        else if (order.status === 'ASSIGNED' || order.status === 'IN_PROGRESS') statusMapped = 'approved';
        else if (order.status === 'COMPLETED') statusMapped = 'completed';
        else if (order.status === 'CANCELLED') statusMapped = 'cancelled';
        ...
        return {
          ...
          latitude: order.latitude || 36.7538,
          longitude: order.longitude || 3.0588,
          addressName: order.address,
          serviceName: order.service?.name || 'Cleaning Service',
          layoutType: order.houseConfig?.type || 'f2',
          defaultWorkers: order.houseConfig?.workers || 3,
          extraWorkers: order.extraWorkers || 0,
          useMaterials: order.useMaterials || false,
          productType: (order.productOrigin || 'NONE').toLowerCase() as any,
          totalPrice: order.totalPrice,
          durationHours: order.service?.durationHours || 4,
          scheduledDate: new Date(order.scheduledDate).toISOString().split('T')[0],
          scheduledTime: new Date(order.scheduledDate).toISOString().split('T')[1]?.slice(0, 5) || '09:00',
          status: statusMapped,
  ...
  const handleUpdateStatus = async (id: string, newStatus: 'pending' | 'approved' | 'completed' | 'cancelled') => {
    let backendStatus = 'PENDING';
    if (newStatus === 'pending') backendStatus = 'PENDING';
    else if (newStatus === 'approved') backendStatus = 'ASSIGNED';
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const token = localStorage.getItem('nadif_admin_token');
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001/api';
      const response = await fetch(`${apiUrl}/orders/${commandToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      if (!response.ok) throw new Error('Failed to delete order on server');
      ...
      console.error(e);
      alert('Error deleting order on backend.');
    }
  };
