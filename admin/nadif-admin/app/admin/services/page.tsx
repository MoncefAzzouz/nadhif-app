...
interface HouseConfig {
  ...
  workers: number;   // e.g. 3, 4, 5
  basePrice: number; // Base price specifically for this layout type in DA
}
...
interface Service {
  ...
  name: string;
  description: string;
  picture: string;          // image asset path or Base64 string
  houseConfigs: HouseConfig[];
  extraWorkerPrice: number; // Price per extra worker in DA
  durationHours: number;    // Default 4 hours
  
  // Materials Surcharge Config
  materialPrice: number;       // Surcharge if using Nadif materials in DA
  materialsMandatory: boolean; // True for Grand/Semi Grand (user cannot deselect)
  
  // Products Surcharge Config
  localProductPrice: number;    // Surcharge if using Nadif local products in DA
  importedProductPrice: number; // Surcharge if using Nadif imported products in DA
  productsMandatory: boolean;   // True for Grand/Semi Grand (cannot select 'own products')
...
const DEFAULT_SERVICES: Service[] = [
  ...
  },
  {
    ...
    durationHours: 4,
    materialPrice: 1500,
    materialsMandatory: true, // MANDATORY: user cannot deselect
    localProductPrice: 1200,
    importedProductPrice: 2000,
    productsMandatory: true, // MANDATORY: user cannot choose 'own products'
    isActive: true
  },
  {
    id: 's3',
...
export default function ServicesPage() {
  ...
  const [houseConfigs, setHouseConfigs] = useState<HouseConfig[]>([]);
  ...
  const fetchServices = async () => {
    try {
      ...
      setServices(data);
      if (data.length > 0) {
        setSelectedSimService(data[0]);
      }
    } catch (e) {
      console.error('Error fetching services from backend:', e);
      const stored = localStorage.getItem('nadif_services');
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          setServices(parsed);
          if (parsed.length > 0) setSelectedSimService(parsed[0]);
        } catch (err) {
          setServices(DEFAULT_SERVICES);
          setSelectedSimService(DEFAULT_SERVICES[0]);
        }
      } else {
        setServices(DEFAULT_SERVICES);
        setSelectedSimService(DEFAULT_SERVICES[0]);
  ...
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem('nadif_services', JSON.stringify(services));
    }
  }, [services, isLoaded]);
  ...
  const handleAddHouseConfig = () => {
    ...
    if (houseConfigs.some(config => config.type === typeCleaned)) {
  ...
  const handleRemoveHouseConfig = (type: string) => {
    setHouseConfigs(prev => prev.filter(c => c.type !== type));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Force materials & products mandatory true for Grand/Semi Grand to prevent misconfiguration
    let forceMaterialsMandatory = materialsMandatory;
    let forceProductsMandatory = productsMandatory;
    const cleanName = name.toLowerCase().trim();
    if (cleanName.includes('grand') || cleanName.includes('semi grand') || cleanName.includes('semi-grand')) {
      forceMaterialsMandatory = true;
      forceProductsMandatory = true;
    }
      ...
      setServices(prev => prev.map(srv => 
        srv.id === editingService.id 
          ? { 
              ...
              durationHours, 
              materialPrice, 
              materialsMandatory: forceMaterialsMandatory, 
              localProductPrice,
              importedProductPrice,
              productsMandatory: forceProductsMandatory,
              isActive 
            } 
          : srv
      ));
  ...
  const getSimCalculations = () => {
    ...
    const config = selectedSimService.houseConfigs.find(c => c.type === selectedSimHouse);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs font-bold outline-none text-white focus:border-primary/50"
                  >
                    {selectedSimService.houseConfigs.map(config => (
