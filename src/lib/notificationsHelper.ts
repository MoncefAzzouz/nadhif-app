 ...
 * change): every user holding the ADMIN role, plus the ADMIN_EMAIL mailbox when
 * one is configured. CUSTOMER and CLEANER accounts are never included — those
 * messages carry another person's name, phone, address and price.
 *
 * The role is therefore the access control: granting ADMIN to an account starts
 * sending it every customer's order details, so keep that role to your team only.
 * Placeholder addresses generated by the backend (phone-only signups, guest
 * customers) are dropped, and duplicates are collapsed case-insensitively.
 */
async function getAdminEmails(): Promise<string[]> {
  // Keyed by lowercased address so the same mailbox is not mailed twice.
  const recipients = new Map<string, string>();
  for (const email of adminNotificationEmails) {
  ...
  try {
    ...
    for (const admin of admins) {
      ...
      if (isDeliverableEmail(email)) recipients.set(email.toLowerCase(), email);
    }
  } catch (error) {
  ...
  if (recipients.size === 0) {
    console.warn(
      '⚠️ No admin notification recipient: no ADMIN user has a valid email and ADMIN_EMAIL is unset.',
    );
  }
